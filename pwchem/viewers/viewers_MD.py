...
_CLUSTER_IMAGES = [
    ('linear', 'Cluster evolution along the trajectory'),
    ('hist', 'Cluster size histogram'),
    ('distmat', 'RMSD distance matrix between frames'),
    ('den', 'Clustering dendrogram'),
    ('dist', 'Relative distance between clusters'),
]
...
class TrajClusteringViewer(SetOfAtomStructViewer):
  """ Viewer for the Trajectory Clustering protocol: displays the clustering
  summary images generated by TTClust and offers the same visualization
  options (PyMol/ChimeraX, alignment, table view) as the SetOfAtomStructs
  viewer for the output representative structures. """
  _label = 'Viewer Trajectory Clustering'
  _targets = [ProtocolTrajectoryClustering]

  def _defineParams(self, form):
    form.addSection(label='Clustering images')
    self._imagesByParam = self._getAvailableImages()
    if self._imagesByParam:
      for paramName, (imgFile, title) in self._imagesByParam.items():
        form.addParam(paramName, params.LabelParam,
                      label='Display {}: '.format(title),
                      help='Open the "{}" plot generated by TTClust.'.format(title))
    else:
      form.addParam('noImages', params.LabelParam,
                    label='No clustering images found in extra/clustering')
    super()._defineParams(form)

  def _getVisualizeDict(self):
    d = super()._getVisualizeDict()
    d.update({paramName: self.showClusterImage for paramName in self._imagesByParam})
    return d

  def showClusterImage(self, paramName=None):
    imgFile, title = self._imagesByParam[paramName]
  ...
  def _getAvailableImages(self):
    clustDir = self.protocol._getExtraPath('clustering')
    images = {}
    if os.path.isdir(clustDir):
      for suffix, title in _CLUSTER_IMAGES:
        matches = glob.glob(os.path.join(clustDir, '*-{}.png'.format(suffix)))
        if matches:
