...
class Plugin(pwem.Plugin):
    ...
    def _defineVariables(cls):
        ...
        cls._defineEmVar(SCIK_DIC['home'], cls.getEnvName(SCIK_DIC))

        # Common enviroments
        cls._defineVar('RDKIT_ENV_ACTIVATION', cls.getEnvActivationCommand(RDKIT_DIC))
        cls._defineVar('SCK_ENV_ACTIVATION', cls.getEnvActivationCommand(SCIK_DIC))
...
######################## PACKAGES #########################
    ...
    @classmethod
    def addScikitPackage(cls, env, default=True):
        """Create clustering environment like RDKit but with a specific scikit-learn version."""
        installer = InstallHelper(
            SCIK_DIC['name'],
            packageHome=cls.getVar(SCIK_DIC['home']),
            packageVersion=SCIK_DIC['version']
        )

        env_path = os.environ.get('PATH', "")
        clustEnvName = cls.getEnvName(SCIK_DIC)
        ...
        installer.addCommand(
            f'{cls.getEnvActivationCommand(SCIK_DIC)} && conda install -c conda-forge scikit-learn-extra -y',
            'SKLEARN_INSTALLED'
        )
        installer.addCommand('mkdir -p oddtModels', 'ODTMODELS_CREATED')
        installer.addPackage(
            env,
            dependencies=['conda'],
            default=default,
            vars={'PATH': env_path} if env_path else None
        )
...
##################### UTILS ###########################
    ...
    def getODDTModelsPath(cls, path=''):
        oddtModelsDir = os.path.abspath(os.path.join(cls.getVar(SCIK_DIC['home']), 'oddtModels'))
