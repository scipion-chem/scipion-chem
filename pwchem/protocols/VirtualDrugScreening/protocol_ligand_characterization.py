...
class ProtocolLigandCharacterization(EMProtocol):
    ...
    def _getLibraryFiles(self, molInput):
        """Get ligand files from a SmallMoleculesLibrary."""
        inDir = os.path.abspath(self._getTmpPath())
        try:
            return molInput.splitInFiles(inDir)
        except Exception as e:
            self.info(f"splitInFiles() failed ({type(e).__name__}: {e}). Trying directory glob.")
            return self._globLibraryFiles(molInput.getFileName())

    def _globLibraryFiles(self, libSrc):
        """Fallback: glob ligand files from a directory."""
        if not os.path.isdir(libSrc):
    ...
    def _buildLibraryDict(self, molInput):
        """Build molDict from SmallMoleculesLibrary."""
        molDict = {}
        for f in self._getLibraryFiles(molInput):
    ...
    def _buildMolDict(self):
        ...
        if isinstance(molInput, SmallMoleculesLibrary):
            return self._buildLibraryDict(molInput)
        return {mol.molName.get(): os.path.join(self.getProject().getPath(), mol.getFileName())
                for mol in molInput}
    ...
    def _attachProperties(self, molCopy, molName, header, propertyDict):
        """Attach descriptor properties to a molecule copy."""
        if molName not in propertyDict:
            return
        ...
        for i, propName in enumerate(header):
            ...
            setattr(molCopy, attrName, Float(row[i]))

    def _createOutputFromLibrary(self, molInput, header, propertyDict):
        """Create output SetOfSmallMolecules from a SmallMoleculesLibrary."""
        ...
        for molName, molPath in self._buildMolDict().items():
            ...
            self._attachProperties(molCopy, molName, header, propertyDict)
            newMols.append(molCopy)
        return newMols

    def _createOutputFromSet(self, molInput, header, propertyDict):
        """Create output SetOfSmallMolecules from a SetOfSmallMolecules."""
        newMols = SetOfSmallMolecules.createCopy(molInput, self._getPath(), copyInfo=True)
        for mol in molInput:
            molCopy = mol.clone()
            self._attachProperties(molCopy, molCopy.molName.get(), header, propertyDict)
            newMols.append(molCopy)
        return newMols
    ...
    def createOutputStep(self):
        ...
        if isinstance(molInput, SmallMoleculesLibrary):
            newMols = self._createOutputFromLibrary(molInput, header, propertyDict)
        else:
            newMols = self._createOutputFromSet(molInput, header, propertyDict)
