...
class ProtChemOBabelPrepareLigands(EMProtocol):
    ...
    def _defineParams(self, form):
        ...
        conformers.addParam('conformer', params.BooleanParam, default=False,
        ...
        conformers.addParam('number_conf', params.IntParam,
                            default=200, condition = "conformer",
                            label='Max. number of conformers:',
                            help='Set the number of conformers generated by OpenBabel from the same molecule.')

        conformers.addParam('rmsd_cutoff', params.FloatParam, condition = "method_conf != 0 and conformer",
                            default=0.5,
                            label='RMSD cutoff:',
                            help='Set the number of conformers generated by OpenBabel from the same molecule.',
                            expertLevel=LEVEL_ADVANCED)
    ...
    def addcharges(self):
        ...
        for filesdf in glob.glob(self._getTmpPath("*")):
            ...
            # With a given pH
            if self.ph.get():
                args = " -isdf %s -p %s --partialcharge %s -O %s-prep.mol2" % (os.path.abspath(filesdf),
                ...
                args = " -isdf %s -h --partialcharge %s -O %s-prep.mol2" % (os.path.abspath(filesdf), cmethod, fnRoot)
                runOpenBabel(protocol=self, args=args, cwd=os.path.abspath(self._getExtraPath()))
