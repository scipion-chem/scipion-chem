...
class ProtocolLigandsFetching(EMProtocol):
    ...
    It enables automated retrieval, filtering, conversion, and structuring of
    protein–ligand interaction data for downstream structural biology and
    virtual screening workflows.

    Inputs
    ------
    inputType:
        Defines the source of the initial query:
        - Uniprot entry (protein-centric search)
        - Target ID (direct target database query)
        - Ligand ID (direct small-molecule query)

    inputDatabaseUniprot:
        Database used when starting from a UniProt entry:
        - PDB, ChEMBL, or BindingDB cross-references

    inputDatabaseTarget:
        Target database when inputType = Target ID:
        - PDB or ChEMBL

    inputDatabaseLigand:
        Ligand database when inputType = Ligand ID:
        - ChEMBL, BindingDB, ZINC, PubChem

    inputIDs / inputIDsStr:
        Set of input identifiers provided either as:
        - SetOfDatabaseID object
        - Comma-separated string list

    fromString:
        Boolean flag indicating whether IDs are provided manually as text.

    Managing Options
    -----------------
    nonRep:
        Removes duplicate ligands based on identifier.

    structDatabase:
        Defines mapping target database for BindingDB ligands:
        - PubChem, DrugBank, or SMILES-only output

    useManager:
        Structure handling backend:
        - RDKit or OpenBabel for conversion/standardization

    Filtering Options
    ------------------
    Experimental structure filters (PDB):
        - Experimental method (X-ray, NMR, EM, etc.)
        - Maximum resolution threshold
        - Deposition date filtering (before/after year)

    ChEMBL assay filters:
        - Assay type (Binding, Functional, ADMET, Toxicity, etc.)
        - Bioactivity type (Ki, Kd, IC50, EC50)
        - Minimum bioactivity threshold (nM)
    ...
    --------
    1. Input parsing
       - Reads UniProt IDs, target IDs, or ligand IDs
       - Supports both object-based and string-based input

    2. Database querying
       - UniProt cross-reference retrieval
       - PDB structure and ligand extraction via RCSB API
       - ChEMBL activity and molecule queries via REST API
       - BindingDB ligand retrieval via web services

    3. Target-to-ligand mapping
       - Extracts target IDs from UniProt entries
       - Retrieves associated ligands from:
         - PDB non-polymer entities
         - ChEMBL activity records

    4. Ligand retrieval and normalization
       - Extracts SMILES (when available)
       - Maps BindingDB IDs to PubChem or DrugBank
       - Downloads structures from:
         - PubChem
         - DrugBank
         - ZINC
         - BindingDB
       - Optionally converts SMILES into 3D structures

    5. Structure processing
       - Parses structures using Biopython MMCIFParser
       - Filters ligands by atom count and molecular properties
       - Removes duplicates if enabled

    6. Optional structure alignment
       - Aligns multiple PDB structures using transformation matrices
       - Uses first structure as reference

    7. Output generation
       - Writes ligand structures to output directory
       - Generates SMILES/SDF/PDB files depending on source
       - Builds SetOfSmallMolecules collection

    8. Logging and traceability
       - Records all mapping relations in idsRelations.txt
       - Logs errors in errors.txt

    Output
    ------
    outputSmallMolecules:
        SetOfSmallMolecules containing:
        - Retrieved ligand structures (SDF/PDB)
        - Metadata derived from source databases
        - Optional molecular annotations

    Summary
    -------
    This protocol enables large-scale automated retrieval of protein–ligand
    interactions across multiple bioinformatics databases.

    It integrates structure-based and ligand-based data sources,
    applies biochemical and structural filtering, and produces
    standardized molecular datasets suitable for docking,
    virtual screening, and downstream computational analysis.

    Notes
    -----
    - Requires internet access for API calls to UniProt, RCSB, ChEMBL,
      BindingDB, PubChem, ZINC, and DrugBank.
    - Uses Biopython, RDKit, or OpenBabel for structure handling.
    - Designed for high-throughput ligand discovery pipelines.
    """
